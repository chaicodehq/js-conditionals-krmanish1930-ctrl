...
export function canBorrowBook(memberAge, hasValidCard, overdueBooks) {
  let r;
  if(memberAge<6 || hasValidCard==false || overdueBooks>0){
    if(memberAge<6){
      r="allowed: false, message: Too young - must be at least 6 years old";
      
    }else if(hasValidCard==false){
      r=" allowed: false, message: Invalid library card - please renew at the front desk";
      
    }else if(overdueBooks>0){
      r=`allowed: false, message: Please return your ${overdueBooks} overdue book(s) first `;
    }
  }else if(memberAge=>6 && hasValidCard==true && overdueBooks==0){
    r="allowed: true, message: You may borrow up to 3 books";
  }
  return r;
}
